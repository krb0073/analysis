...
if [ $# -eq 0 ]; then
     echo "sh $0 benchamrk|info|chain|restart|exstend|help steps time queue run_name"
     exit
     ...
     echo "run_name - base name for simulations "
     exit
fi

run=$1; total_steps=$2; wall_hours=$3; queue=$4 ;run_name=$5
...
if [ $run = 'benchmark' ]; then
     ...
     sed -i "s|insert_run_num|0|g" benchmark/pbs.sh
     # Insert walltime
     sed -i 's|insert_walltime_here|1|g' benchmark/pbs.sh
     # submit pbs
     cd benchmark
     qsub pbs.sh
     cd $WORK_DIR
     ...
     if ! [ -f benchmark/run.log ]; then
          ...
          seconds_per_step=$(grep Benchmark benchmark/run.log |cut -f 6-6 -d ' ') ; echo $seconds_per_step
          val=0; cnt=0; for i in $seconds_per_step ; do vval=$(echo "print(float($val) + float($i))"| python);val=$vval; cnt=$(($cnt +1)) ; done ; avg=$(echo "print($val / 6.0) "|python)
          # Convert to steps per hour
          steps_per_hour=$(echo "print(3600.0/float($avg))" |python)
          ...
          number_of_jobs=$(echo "import math; print(math.ceil(float($total_steps)/float($steps_per_job)))" |python)

          echo "With the present parameters, the maximum steps completed per job is $steps_per_job steps."
	...
	# Get seconds per step from benchmark log
          seconds_per_step=$(grep Benchmark benchmark/run.log |cut -f 6-6 -d ' ')
          val=0; cnt=0; for i in $seconds_per_step ; do vval=$(echo "print(float($val) + float($i))"| python);val=$vval; cnt=$(($cnt +1)) ; done ; avg=$(echo "print($val / 6.0) "|python)
          # Convert to steps per hour
          steps_per_hour=$(echo "print(3600.0/float($avg))" |python)
          # Determine steps per job
          steps_per_job=$(echo "import math; print(int(math.floor(float($steps_per_hour)*float($wall_hours)/1000.0)*1000))" |python)
          # Determine number of chains
          number_of_jobs=$(echo "import math; print(math.ceil(float($total_steps)/float($steps_per_job)))" |python)
	      ...
	      sed -i "s|insert_run_num|${n}|g" runs/${n}/pbs.sh
               # Insert walltime
               sed -i "s|insert_walltime_here|${wall_hours}|g" runs/${n}/pbs.sh
               # Make a run config
          ...
          sed -i 's|insert_firsttimestep_here|0|g' runs/1/run.conf
	  sim_name=$(echo "${run_name}_1")
	  sed -i "s/insert_name_here/${sim_name}/g" runs/1/pbs.sh 
          cd runs/1/
          ...
          for n in $(seq 2 $number_of_jobs); do
               cd runs/$n/
               sim_name=$(echo "${run_name}_${n}")
               sed -i "s/insert_name_here/${sim_name}/g" pbs.sh
               echo -n "$n "
	...
	# Get seconds per step from benchmark log
          ...
          steps_per_hour=$(echo "print(3600.0/float($avg))" |python)
          # Determine steps per job
          steps_per_jobs=$(echo "import math; print(math.floor(float($steps_per_hour)*float($wall_hours)/1000.0)*1000)" |python)
	  job_steps=$(echo $steps_per_jobs | awk -F.'{print $1}')
	  echo $job_steps
	  exit
          # Determine number of chains
          number_of_jobs=$(echo "import math; print(math.ceil(float($total_steps)/float($steps_per_job)))" |python)
	# last good run done for the procject 
	  last_good_run=$(ls -1v runs/*/*.dcd | sed 's/\// /g'|tail -n 1|awk '{print $2}') # this gets the dir number that actaully worked
	...
	sim_name=$(echo "${run_name}_${failed_starting_index}")
	 sed -i "s/insert_name_here/${sim_name}/g" runs/$failed_starting_index/pbs.sh	
	...
	cd $WORK_DIR
	  ...
	  for n in $(seq $next_run $number_of_jobs); do
		sim_name=$(echo "${run_name}_${n}")
		sed -i "s/insert_name_here/${sim_name}/g" runs/$n/pbs.sh
               echo -n "$n "
               # Submit the subsequent jobs depending on the previous one
		cd runs/$n/
		two=$(qsub -W depend=afterany:$one pbs.sh)
...
elif [ $run == "exstend" ] ; then 
	# need to check for both the run benchmark are there 
	if ! [ -f benchmark/run.log ] || ! [ -d runs/ ] ;then
		echo "need to run either benchmark or chain to exstend the traj. ... exiting"
		exit
	else
	  # Get seconds per step from benchmark log
          seconds_per_step=$(grep Benchmark benchmark/run.log |cut -f 6-6 -d ' ')
          ...
          # Determine steps per job
          steps_per_job=$(echo "import math; print(math.floor(float($steps_per_hour)*float($wall_hours)/1000.0)*1000)" |python)
	  job_steps=$(echo $steps_per_job |awk -F. '{print $1}')

          # Determine number of chains
          number_of_jobs=$(echo "import math; print(math.ceil(float($total_steps)/float($steps_per_job)))" |python)
	  echo $number_of_jobs
          mod_job_num=$(echo $number_of_jobs | awk -F. '{print $1}')   

       	  # we need to find the last job ran in runs 
	 last_run=$(ls -1v runs/|grep -v job_ids.txt |tail -n 1) 
	# the next run would then be last_runs ++ 
	next_run=$(( $last_run + 1 ))
	# the final run is next_run + number_of_jobs	
	final_run=$(( $next_run + $mod_job_num ))
	          # Loop to create pbs and submit chained jobs
          mkdir -p runs
          for n in $(seq $next_run $final_run); do
               mkdir -p runs/$n
               ...
               cp common/template.conf runs/${n}/run.conf
               # # of steps
               sed -i "s|insert_steps_here|${job_steps}|g" runs/${n}/run.conf
               # Common location
               sed -i "s|insert_common_here|${COMMON}|g" runs/${n}/run.conf
               # Number of the job
               sed -i "s|insert_jobnum_here|${n}|g" runs/${n}/run.conf
      ...
      # Submit the first job in the chain
          sed -i "s|insert_firsttimestep_here|0|g" runs/${next_run}/run.conf
          sim_name=$(echo "${run_name}_${next_run}")
          sed -i "s/insert_name_here/${sim_name}/g" runs/$next_run/pbs.sh
          cd runs/$next_run
          echo -n "Submitting job ${next_run} "
          one=$(qsub pbs.sh)
          cd $WORK_DIR
          # Add the job id to the list
          echo $one >> runs/job_ids.txt
	next_next_run=$(( $next_run + 1))
   for n in $(seq $next_next_run $final_run); do
               cd runs/$n/
               sim_name=$(echo "${run_name}_${n}")
               sed -i "s/insert_name_here/${sim_name}/g" pbs.sh
               echo -n "$n "
