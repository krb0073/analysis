...
# @Date:   2020-07-20T11:13:19-04:00
# @Email:  krb0073@mix.wvu.edu
# @Filename: LR_K-fold_and_ridge.py
# @Last modified by:   kbillings
# @Last modified time: 2020-08-07T18:46:18-04:00
# @Author: Kyle Billings <kbillings>
...
# making the data frames for all the stuff
...
Rn.fit(X,y)
#for item in range(len(X.columns)):
#    print(list(X.columns)[item],Rn.coef_[item])
# test with only two things
#print("prediction of the train data")
#print(Rn.predict(X))
#print("the score for the model")
#print(Rn.score(X,y))
#print("Mean Square error for all",mean_squared_error(y, Rn.predict(X)) )
#l = ["C18C5C6C1","C4C5C6C1"]
#ridgecv.fit(X[l],y)
#A = ridgecv.alpha_
#Rn.set_params(alpha = A)
#Rn.fit(X[l],y)
#print("only with {0} with best alpha we get {1}".format(l,A))
#print(l ,Rn.coef_)
#print("predicted results")
#print(Rn.predict(X[l]))
#print("R squared vals", Rn.score(X[l],y))
#print("Mean Square error for all",mean_squared_error(y, Rn.predict(X[l])) )
#print(Rn.get_params)
import time
print("testing the k best model")
tic = time.time()
print(ridgeNoKfold_2(df3,y,2))
tock = time.time()
print(tock - tic)
#print(ridge.fit(X,y))
#print(ridge.coef_)
#setList = [df1,df2,df3,df4, df5,df6 ,df7,df8 ,df9 ,df10]
#ridgecv = RidgeCV(alphas = alphas, scoring = 'neg_mean_squared_error', normalize = True)
#Rn = Ridge(normalize = True)
#k= 2
...
#    for D1 in setList:
#        cnts += 1
#        X1 = D1.drop("Lmax", axis = 1)
#        X1 = X1[list(combo)]
#        MC = np.zeros((len(combo)+2))
#        if k == 1:
#            X1 = np.array((X1)).reshape(-1,1)
#        cnt2 = 1
#        for D2 in setList:
#            tempMC = np.zeros((len(combo)+2))
#            if cnts != cnt2:
#                res = []
#                X2 = D2.drop("Lmax", axis = 1)
#                X2 = X2[list(combo)]
#                if k == 1:
#                    X2 = np.array((X2)).reshape(-1,1)
#                mods = ridgecv.fit(X2,y)
#                lmin = mods.alpha_
#                p = 0
#                tempMC[p] = lmin
#                Rn.set_params(alpha = lmin)
#                Rn.fit(X2,y)
#                p = 1
#                tempMC[p] = Rn.intercept_
#                p += 1
                #print(combo ,cnts,cnt2,Rn.intercept_,Rn.coef_)
#                for c in list(Rn.coef_):
#                    tempMC[p] = c
#                    p += 1
#                MC = MC  + tempMC
                #print(myR ,list(combo), Rn.intercept_ , cnts)
#            cnt2 += 1
#    MC = MC / 90 # avg value
#    print(combo,"lamba,yint,coef[1--N]" ,MC)
