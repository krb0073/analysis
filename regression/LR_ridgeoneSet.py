...
# @Date:   2020-07-22T22:45:45-04:00
# @Email:  krb0073@mix.wvu.edu
# @Filename: LR_ridgeoneSet.py
# @Last modified by:   kbillings
# @Last modified time: 2020-08-13T16:27:54-04:00
import sys
import numpy as np
import os
...
for k in range(1,5):
    print(k)
    res.append(ridgeNoKfold(df3,y,k))
final = pd.DataFrame(res)
print(final)
TestingData = []
setList = [df1,df2,df4, df5,df6 ,df7,df8 ,df9 ,df10]
for mod in final.itertuples():
    name = 1
    for sets in setList:
        X2 = sets
        X2 = X2.drop("Lmax",axis=1)
        Rn = mod[1]
        vars = list(mod[2])
        Rn.fit(X[vars],y)
        RSS = (((Rn.predict(X2[vars])-y)**2).sum() )
        pre = Rn.predict(X2[vars])
        dics = {"DataFrame": name, "Var": vars , "result": list(pre) , "RSS" : RSS }
        TestingData.append(dics)
        name += 1
test = pd.DataFrame(TestingData)
#print(test)
