...
def main(config_file):
    ...
    reg = OLS_regression(config.data_range[0],config.data_range[1],config.Lmax,config.dirloc,config.seedNum,config.num_var)
    ...
    reg.plot_distrabution(data,config.n_sets,config.systems_list,mix=config.mix)
    # add in the AUC if needed
    if config.path_auc != None:
    ...
    if config.method == "LOOCV":
        ...
        if config.untouched_num != None:
            reg.untouchedData(nset=config.untouched_num)
        reg.kfold_runner(tolerance=config.data_tol,purge_cutoff=config.purge_cutoff,cv_metric=config.metric,flipp=config.flipp)
main(config_file)
