...
# @Date:   2020-08-13T12:17:52-04:00
# @Email:  krb0073@mix.wvu.edu
# @Filename: LR_kfold_ridgeSklearn.py
# @Last modified by:   kbillings
# @Last modified time: 2020-09-23T17:27:35-04:00
import sys
import numpy as np
import os
...
Ys = allDF["Lmax"]
mods = []
X = set[0].drop("Lmax",axis=1)
Y = set[0]['Lmax']
#DF.remove(set_to_use)
testing_data =  pd.concat(DF,ignore_index=True)

for i in range(1,5):
    mods.append(Kfold_skRidge_10precent(allDF,Ys,i,metric="MSE"))
mods = pd.DataFrame(mods)
All_A = list(mods['Alpha'])
cnt = 0
for combo in list(mods['combo']):
    title = str(path) + 'Kfold10precent_Ridge_' +"".join(combo) + ".png"
    A = All_A[cnt]
    cnt += 1
    ...
    print("RSS" , mets["RSS"])
    act_model = list(Y)
    pred_kfold ,act_kfold = precent_10_kfold_ridge_grab_all(allDF,Ys,A,combo)
    # grab the equation
    m = mets["model"]
    ...
    plt.legend(('model | R squared ' + str(r_value) , '10fold cv| R sqaured ' + str(r_value2)) )
    plt.savefig(title)
#plt.show()
