...
class Regression_setup:
    ...
    def __init__(self,start_index,end_index,LMAXS,Data_location,seedNum,skip=0):
    ...
    def Make_data_frames(self,prefix,suffix='dat',skip=0):
        ...
        self.measures = []
        for f in filenames:
            ans = f.split("/")[-1].split("_")[-1].split(".")[0] # this will always get a file name
            ...
            with open(file) as txt_file:
                ...
                for line in txt_file: # loop into each line
                    ...
                    if skip != 0: # n , n+skip , n+skip*2 ... till we are near or past the target
                        ...
                        line_num += 1
                if file == filenames[0]: # strating out i need a fresh frame to make the data set
                    data = np.array(temp_data)
                else:
                    data = np.row_stack((data,temp_data)) # from then on i stack the rows on for each file
    ...
    def Create_final_dataset_splits(self,list_of_np_arrays,number_of_datasets,mix=True):
        ...
        self.mix = mix
        if self.mix == True:
            set_1 = scramble(set_1,self.seedNum,axis=1) # each row is one set of measure can not scrable the whole thing
        try:
        ...
        if self.n_folds != 1: # if we want to use one set then this is not needed
            self.model_index = random.randint(0,self.n_folds -1) # picking one index to be main boi
    ...
    def plot_distrabution(self,list_of_np_arrays,number_of_datasets,systems_list,mix=True):
        ...
        for H,B in zip(hists,bins): # all varaibles
            ...
            ax.legend(loc='best',prop={"size":25}).set_title(name,prop={'size':25})
        fig.tight_layout()
...
class OLS_regression(Regression_setup):
    ...
    def kfold_runner(self,tolerance=0,flipp=False,purge_cutoff=None,cv_metric='MSE'):
        ...
        for par in good_par: # now will build the model with the one we want (Meesy)
            ...
            ax.legend(prop={"size":25},loc='best',framealpha=0.25).set_title(list(par), prop={'size':25})
        fig.tight_layout()
