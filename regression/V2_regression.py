...
# @Last modified time: 2021-08-24T12:35:00-04:00
# found this scarmbel code at
...
class readConfig:
    ...
    def __init__(self, File):
        ...
        for line in file.readlines():
            ...
            if line.startswith("#") or line.isspace() or len(
                    line) == 0 or "@" in line:
                continue
                ...
                if checker > 2:
                    # we have a number to skip by so we will append that in as
                    # well
                    self.data_range.append(float(line.split()[3]))
            elif line.upper().startswith("LMAX"):
                vals = line.split()[
                    1:]  # this will get one to the end w/o lmax
                for v in vals:
                    self.Lmax.append(int(v))
            elif line.upper().startswith("DIR_NAME"):  # name of the DIR to use
                ...
                )[1:]:  # loop from the frist prefix to the end
                ...
                if fp == "True":
                    self.flipp = True
            elif line.upper().startswith("NO_MIX"):
                self.mix = False
        ...
        if self.num_var is None:
            print(
                "missing the total of number of varables to try (e.i. 1 ,2 ) ...exiting")
        if len(self.Lmax) == 0:
        ...
        if len(self.Lmax) == len(self.sim_list):
            if len(self.sim_list) == len(self.systems_list):
                ...
                print(
                    "Lenght of systems list is not the same as lmax, or prefix list... exiting \n")
                self.config_error_message()
        else:
            print("Lenght of Lmax and prefix list not equal... exiting \n")

        if self.seedNum is None:
            print("No random seed given  data will not be randomized\n")
            self.config_error_message()
        if self.method != "LOOCV" and self.method != "KFOLD":
            print("Mehod Type is not correct/not given ... exting\n")
            self.config_error_message()
        # working with the optinal stuff will not cause (in most cases the
        # failing)
        if self.data_tol is None:
            print("No tolerance is given all data being used")
            self.tolerance = 0
        if self.purge_cutoff is None:
            print("correaltions are not being trimmed setting pruge to defult ")
            self.purge_cutoff = 1
        if self.n_sets is None:
            ...
            if self.keep_num is not None:
                print(
                    "the untouched data set is not for uaing one big set ... esiting\n")
                sys.exit(1)
            else:
                self.keep_num = 5
        if self.metric == "MSE":
        ...
        if self.method == "LOOCV" or self.method == "KFOLD":  # if we good here
            if self.keep_num is not None or self.untouched_num is not None:
                if self.n_sets is None:  # this is if the user wanted to keep with spliting up
...
class Regression_setup:
    ...
    def RandomSeed(self, path=None):
        ...
        try:
            ...
            sys.exit()
        self.dir_path = path + "/" + \
            f"Regression_results_for_seed_{self.seedNum}"
    ...
    def Make_data_frames(self, prefix, suffix='dat', skip=0):
        ...
        # TODO: move the RMSF under the curve with the rest of the data and fix
        # format
        file_loc = os.path.abspath(
            self.Data_location)  # gets full path of data
        print(file_loc + f"/{prefix}*.{suffix}")
        # list all the prefix*.suffix files to use
        filenames = glob.glob(file_loc + f"/{prefix}*.{suffix}")
        n_cols = len(filenames)
        n_rows = self.start_index
        self.measures = []
        for f in filenames:
            # this will always get a file name
            ans = f.split("/")[-1].split("_")[-1].split(".")[0]
            self.measures.append(ans)
        # i.e. if i want bR*.dat I could get all of those files in a list
        print(
            f"The follewing file will be used to run linear regression of {prefix}:")
                ...
                for line in txt_file:  # loop into each line
                    if line_num >= self.start_index and line_num <= self.end_index:  # checkin if we are at the right index
                        if line.startswith("#") or line.isspace() or len(
                            ...
                            # append measurment as a float
                            temp_data.append(float(value))
                    if skip != 0:  # n , n+skip , n+skip*2 ... till we are near or past the target
                        line_num += skip
                    else:
                        line_num += 1
                # strating out i need a fresh frame to make the data set
                if file == filenames[0]:
                    data = np.array(temp_data)
                else:
                    # from then on i stack the rows on for each file
    ...
    def Create_final_dataset_splits(
            self,
            list_of_np_arrays,
            number_of_datasets,
            mix=True):
        ...
        self.n_folds = number_of_datasets  # set the number of sets we want
        all_data = list_of_np_arrays  # this is silly vut i it help my brain
        set_1 = all_data[0]  # this is the fiest array in the list
        self.mix = mix
        if self.mix:
            # each row is one set of measure can not scrable the whole thing
            set_1 = scramble(set_1, self.seedNum, axis=1)
        try:
            n_temp_array = np.hsplit(set_1, self.n_folds)
        except BaseException:
            print(
                f"The number of sets wanted is not disiavble by {self.n_folds}")
            sys.exit(1)
        # using the first to build the array
        n_array = []
        # moving the pick array to here
        self.model_index = 0
        if self.n_folds != 1:  # if we want to use one set then this is not needed
            self.model_index = random.randint(
                0, self.n_folds - 1)  # picking one index to be main boi
            print(
                f"n_fold is lager than 1 using set  index {self.model_index}")
        ...
        for d2 in all_data[1:]:
            temp_array = d2
            if self.mix:
                temp_array = scramble(d2, self.seedNum, axis=1)
            # spilt into the sets we want
            d2_array = np.hsplit(temp_array, self.n_folds)
    ...
    def plot_distrabution(
            self,
            list_of_np_arrays,
            number_of_datasets,
            systems_list,
            mix=True):
        "this function pliots the histogram of the vaules that were used to find the median values"
        ...
        for data in all_data[1:]:
            if self.mix:
                data = scramble(data, self.seedNum, axis=1)
            data = np.hsplit(data, self.n_folds)[self.model_index]
            h = np.apply_along_axis(
                lambda a: np.histogram(
                    a, bins=10, density=True)[0], 1, data)
            b = np.apply_along_axis(
                lambda a: np.histogram(
                    a, bins=10, density=True)[1], 1, data)
        ...
        for H, B in zip(hists, bins):  # all varaibles
            ...
            cnt += 1
            alpha = 1
            # setup the plotter for the things
            ax = fig.add_subplot(rows, cols, cnt)
            ax.set_xlabel('angel', fontsize=25)
            ax.set_ylabel('probablity', fontsize=25)
        ...
        fig.savefig(
            self.dir_path +
            '/' +
            'metrics_histograms.png')  # ,dpi=400)
    ...
    def get_AUC(self, AUC_path, order_of_systems):
        ...
        for run in range(1, self.n_folds + 1):
            ...
            for system in order_of_systems:
                filename = full_path + \
                    f"/{system}_{self.seedNum}_set_{run}.dat"
                # print(filename)
            ...
            self.med_array[run -
                           1] = np.column_stack((self.med_array[run - 1], AUC))
...
class OLS_regression(Regression_setup):
    ...
    def __init__(
        ...
        self.keep = keep_num
        # allows the regression to get all things of Regression_setup
        super().__init__(start_index, end_index, LMAXS, Data_location, seedNum)
    ...
    def pandafy(self, systems):  # this works
        ...
        print("priting values from the final model ")
        print(self.data_frames[self.model_index])
    ...
    def best_subset_runer(self, var_num):
        ...
        cols = self.data_frames[self.model_index].drop(
            "Lmax", axis=1).columns  # list all open columns to do regression
        return [x for x in itertools.combinations(cols, var_num)]

    def peasron_correaltion_heatmap(self):
        "genrates a heat map of the pearson corre4altion of the data"
        non_lmax_data = self.data_frames[self.model_index].drop('Lmax', 1)
        filename = f"heat_Map_{self.seedNum}_for_final_set.png"
        fig = plt.figure(figsize=(20, 20))
        heatmap = sns.heatmap(
            non_lmax_data.corr().abs(),
            cmap='coolwarm',
            vmin=0,
            vmax=1,
            annot=False)
        fig.savefig(self.dir_path + '/' + filename, dpi=100)
    ...
    def tolerance_cheack(self, tolerance):
        ...
        for col in self.data_frames[self.model_index].columns:
            ...
            temp = (self.data_frames[self.model_index][col].values)
            # substract each values from the rest of the arrays. if the difference of the values are not in
            # tolerance range cahnge traker to bad if one vaule is too close
            # then the whole thing is left out
            for val in temp:
                test = abs(temp - val)
                rest = []
                for num in test:
        ...
        for frame in range(0, len(self.data_frames)):
            # for bad in Bad_cols:
            self.data_frames[frame] = self.data_frames[frame].drop(
                columns=Bad_cols, axis=1)
        print(
            f"found {len(Bad_cols)} parameters that where the points were < {tolerance}")
        print(f"removed:")
        print("\n".join(Bad_cols))
    ...
    def purge_corr_too_high(self, var_num, cut_off):
        ...
        if len(bad_combos) != 0:
            print(
                f"found {len(bad_combos)} combanation of data that are above the correaltion threshold {cut_off}")
    ...
    def SST(self):
        ...
        self.SST_val = ((self.data_frames[self.model_index]['Lmax'] -
                         self.data_frames[self.model_index]['Lmax'].mean())**2).sum()
    ...
    def process_subset_statsmodels(self, data_frame, cols):
        ...
        if len(cols) != 1:
            # define x for the model suing the cols that we want
            x = data_frame.drop('Lmax', 1)[list(cols)]
        else:
            x = data_frame.drop('Lmax', 1)[cols[0]]
        Y = data_frame['Lmax']
        # for this versioon to hvae a intercept we need to add a constant
        x = sm.add_constant(x)
        model = sm.OLS(Y, x)
        fit_data_model = model.fit()
        return {
            'model': fit_data_model,
            'combo': list(cols),
            'adj_R': fit_data_model.rsquared_adj,
            "R": fit_data_model.rsquared,
            'BIC': fit_data_model.bic,
            'AIC': fit_data_model.aic,
            'RSS': self.cal_RSS(
                fit_data_model.predict(x),
                Y)}
    ...
    def leave_one_out(self, data_frame, combo, cv_metric='MSE'):
        ...
        return np.mean(cv_array), np.std(cv_array) / \
            (len(cv_array)**0.5), (1 - (PRESS / self.SST_val))[0], p

    def loocv_for_one_variable(
            self,
            data_frame,
            var,
            purge_cutoff=None,
        ...
        LOOCV"""
        if purge_cutoff is not None and var != 1:
        ...
        pred = []
        for combo in combos:
            val, _, pred_R, _ = self.leave_one_out(
                data_frame, combo)  # run leave one out
            cv.append(val)
            pred.append(pred_R)
    ...
    def run_leave_one_out(
        ...
        for num_par in range(1, self.max_var + 1):
            top_n_cv = self.loocv_for_one_variable(
                self.data_frames[self.model_index], num_par, purge_cutoff=purge_cutoff, cv_metric=cv_metric)
            ...
            for p in top_n_cv['pred_rsquared']:
                pred_R.append(p)
        # we have to rerun the top models with the full set to get the final
        # models
        for par, pred in zip(good_par, pred_R):
            result = self.process_subset_statsmodels(
                self.data_frames[self.model_index], par)
            model = result['model']
            print(model.summary())
            eq = self.gen_linear_equation(result)
            print(f"the eqeuation for {par} is:")
            print(eq)
            print(f"The predicted rsqaured vaules is {pred}")
        ...
        for c in good_par:
            cnt += 1
            result = self.process_subset_statsmodels(
                self.data_frames[self.model_index], c)
            data_not_used = []
            expermental_lmax = list(
                self.data_frames[self.model_index]['Lmax']) * (len(self.data_frames) - 1)
            for mod in range(len(self.data_frames)):
                if mod != self.model_index:
                    test_data = list(
                        self.predict_vaules(
                            result, self.data_frames[mod]))
                    for point in test_data:
                        data_not_used.append(point)
            if len(data_not_used) != 0:
                mse_not_used = mean_squared_error(
                    expermental_lmax, data_not_used)
                rmse_test = (mse_not_used)**0.5
                print(f'rmse for combo {c}: {rmse_test}')
            pred = self.predict_vaules(
                result, self.data_frames[self.model_index])
            eq = self.gen_linear_equation(result)
            ax = fig.add_subplot(nrows, ncols, cnt)
            _, _, _, loocv_data = self.leave_one_out(
                self.data_frames[self.model_index], c)
            ax.scatter(self.data_frames[self.model_index]
                       ['Lmax'], pred, c="black", label="model")
            if len(data_not_used) != 0:
                ax.scatter(
                    expermental_lmax,
                    data_not_used,
                    c='red',
                    marker="P",
                    label=f'test-data|RSME {rmse_test}')
            ax.scatter(self.data_frames[self.model_index]['Lmax'],
                       loocv_data, c='blue', marker="*", label='loocv')
            m, b, r_value, p_value, std_err = scipy.stats.linregress(
                self.data_frames[self.model_index]['Lmax'], pred)
            m2, b2, r_value2, p_value2, std_err2 = scipy.stats.linregress(
                self.data_frames[self.model_index]['Lmax'], loocv_data)
            ax.plot(self.data_frames[self.model_index]['Lmax'],
                    m * self.data_frames[self.model_index]['Lmax'] + b,
                    'black',
                    label='model | R squared ' + str(r_value**2))
            ax.plot(self.data_frames[self.model_index]['Lmax'],
                    m2 * self.data_frames[self.model_index]['Lmax'] + b2,
                    'blue',
                    linestyle=":",
                    linewidth=1,
                    label='loocv | R squared ' + str(r_value2**2))
    ...
    def untouchedData(self, nset=5):
        ...
        keeping = []
        # making sure that index is 0
        keeping.append(self.data_frames[self.model_index])
        # changing the index of the model index
        # list all possable indexes
        index_list = [x for x in range(len(self.data_frames) - 1)]
        ...
        # get the rest of the using set
        rans = np.random.choice(keep_index, remain, replace=False)
        testing_index = []
        ...
        self.untouched = test
        # change the number of fold to equal the numder
        self.n_folds = len(self.data_frames)
    ...
    def KfoldCV(self, combo, flipp=False, metric="MSE"):
        ...
        for train, test in cv.split(data):
            ...
            pred = self.predict_vaules(result, te)
            temp_p.append(pred)
            ...
            BIC = self.calc_bic(len(te["Lmax"]), MSE, len(
                result.get('model').params))
            AIC = self.calc_aic(len(te["Lmax"]), MSE, len(
                result.get('model').params))
    ...
    def kfold_runner(
        ...
        pred_R = []
        self.SST()
        for num_par in range(
                1, self.max_var + 1):  # run all the number of parmaters
            top_n_cv = self.kfold_for_one_variable(
                num_par, flipp=flipp, purge_cutoff=purge_cutoff)
            for x in top_n_cv['combo']:
                if len(x) != 1:
                    x_list = list(x)
                else:
        ...
        fig = plt.figure(figsize=(50, 50))
        cnt = 0
        # now will build the model with the one we want (Meesy)
        for par in good_par:
            if flipp:  # this is the case where we are using only ten perent of the model
                modset = self.data_frames[self.model_index]
                try:
                    len(self.untouched)
                except BaseException:
                    print(" you are using a 10% model without leaving data out")
                ...
                _, _, kfoldData = self.KfoldCV(
                    par, flipp=flipp, metric=cv_metric)
                kfold_expre = list(self.data_frames[self.model_index]['Lmax']) * (
                    len(self.data_frames) * (len(self.data_frames) - 1))
                kfold_expre = np.array((kfold_expre), dtype='float64')

            else:
                # will mush the used data into one set for later
                modset = pd.concat(self.data_frames)
                # this case would require that we have a set held back before
                # we finilze the model
                try:
                    len(self.untouched)
                except BaseException:
                    print(
                        "you are using the normal fold method and not doing a test train split ")
                    print(
                        "need vaules for testing the model. pelase run untouchedData and try again")
                    sys.exit(1)
                _, _, kfoldData = self.KfoldCV(
                    par, flipp=flipp, metric=cv_metric)
                kfold_expre = modset['Lmax']
            # with the data now frimily de fined we can build the model next
            # time
            cnt += 1
            ...
            rmse_test = (mse_not_used)**0.5
            print(f'rmse for combo {par}: {rmse_test}')
            ...
            m, b, r_value, p_value, std_err = scipy.stats.linregress(
                modset['Lmax'], pred)
            m2, b2, r_value2, p_value2, std_err2 = scipy.stats.linregress(
                expermental_lmax, test_data)
            m3, b3, r_value3, p_value3, std_err3 = scipy.stats.linregress(
                kfold_expre, kfoldData)
            ax.plot(modset['Lmax'], m *
                    modset['Lmax'] +
                    b, c='black', label='model | R squared ' +
                    str(r_value**2))
            #ax.plot(modset['Lmax'], m2 * modset['Lmax'] + b2 , c ='red',linestyle="--",label='loocv | R squared ' + str(r_value2**2))
            ax.plot(kfold_expre,
                    kfold_expre * m3 + b3,
                    c="red",
                    label="Kfold_cv |R_squared" + str(r_value3**2),
                    linestyle="-.",
                    alpha=0.25)
            #ax.set_title(list(par), fontsize=8)
            # if cnt == ceil(nrows/2):
            ax.set_ylabel("predicted Lmax", fontsize=25)
            # if cnt == ceil(ncols/2):
            ax.set_xlabel("expermental lmax", fontsize=25)
            ax.legend(
                prop={
                    "size": 25},
                loc='best',
                framealpha=0.25).set_title(
                list(par),
                prop={
                    'size': 25})
        fig.tight_layout()
        plt.savefig(self.dir_path + '/' + 'kfold.png')
